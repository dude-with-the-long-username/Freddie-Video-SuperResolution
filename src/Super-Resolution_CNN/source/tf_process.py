...
def training(sess, neuralnet, saver, dataset, epochs, batch_size):
    ...
    for epoch in range(epochs):
        ...
        while(True):
            ...
            summaries, _ = sess.run([neuralnet.summaries, neuralnet.optimizer], feed_dict={
                                    neuralnet.inputs: X_tr, neuralnet.outputs: Y_tr})
            loss_tr, psnr_tr = sess.run([neuralnet.loss, neuralnet.psnr], feed_dict={
                                        neuralnet.inputs: X_tr, neuralnet.outputs: Y_tr})
        ...
        X_tmp, Y_tmp = np.expand_dims(
            X_tr[0], axis=0), np.expand_dims(Y_tr[0], axis=0)

        img_recon, tmp_psnr = sess.run([neuralnet.recon, neuralnet.psnr], feed_dict={
                                       neuralnet.inputs: X_tmp, neuralnet.outputs: Y_tmp})
        img_input, img_recon, img_ground = np.squeeze(X_tmp, axis=0), np.squeeze(
            img_recon, axis=0), np.squeeze(Y_tmp, axis=0)
        ...
        plt.tight_layout(pad=1, w_pad=1, h_pad=1)
        plt.savefig("%s/training/%09d_psnr_%d.png" %
                    (PACK_PATH, epoch, int(tmp_psnr)))
        plt.close()

        """static img(test)"""
        img_recon, tmp_psnr = sess.run([neuralnet.recon, neuralnet.psnr], feed_dict={
                                       neuralnet.inputs: X_static, neuralnet.outputs: Y_static})
        list_psnr_static.append(tmp_psnr)
        img_recon = np.squeeze(img_recon, axis=0)
        plt.imsave("%s/static/reconstruction/%09d_psnr_%d.png" %
                   (PACK_PATH, epoch, int(tmp_psnr)), img_recon)

        print("Epoch [%d / %d] | Loss: %f  PSNR: %f" %
              (epoch, epochs, loss_tr, psnr_tr))
    ...
    save_graph(contents=list_loss, xlabel="Iteration",
               ylabel="L2 loss", savename="loss")
    save_graph(contents=list_psnr, xlabel="Iteration",
               ylabel="PSNR (dB)", savename="psnr")
    save_graph(contents=list_psnr_static, xlabel="Iteration",
               ylabel="PSNR (dB)", savename="psnr_static")
...
def validation(sess, neuralnet, saver, dataset):
    ...
    for tidx in range(dataset.amount_te):
        ...
        img_recon, tmp_psnr = sess.run([neuralnet.recon, neuralnet.psnr], feed_dict={
                                       neuralnet.inputs: X_te, neuralnet.outputs: Y_te})
        img_recon = np.squeeze(img_recon, axis=0)
        plt.imsave("%s/test/reconstruction/%09d_psnr_%d.png" %
                   (PACK_PATH, tidx, int(tmp_psnr)), img_recon)
...
def upscale_np_frame(sess, neuralnet, saver, frame):
    if(os.path.exists(PACK_PATH+"/Checkpoint/model_checker.index")):
        saver.restore(sess, PACK_PATH+"/Checkpoint/model_checker")

    start_time = time.time()
    print("\nUpscale numpy array")
    # TODO: maybe need to do np.expand_dims on frame like in next_test function
    # <13-06-20, Balamurali M> #

    img_recon = sess.run([neuralnet.recon], feed_dict={
                         neuralnet.inputs: frame})
    img_recon = np.squeeze(img_recon, axis=0)
